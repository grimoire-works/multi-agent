...
function findTemplatesDir(): string | null {
  ...
  const possiblePaths = [
    path.resolve(process.argv[1] ?? __dirname, '..', '..', 'skills', 'multi-agent-init', 'templates'),
    path.resolve(process.cwd(), 'skills', 'multi-agent-init', 'templates'),
    path.resolve(__dirname, '..', '..', 'skills', 'multi-agent-init', 'templates'),
  ];

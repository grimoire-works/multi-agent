...
3. 委托 `{代号}-designer` 根据 PRD 出设计稿到 `doc/design/`，展示给用户确认
4. 委托 `{代号}-dev` 根据 PRD + 设计稿出技术方案（涉及模块、技术选型、数据流、影响范围、风险点），写入 `doc/tech-plan.md`，展示给用户确认
5. 委托 `{代号}-pm` 根据确认后的 PRD + 设计稿 + 技术方案拆解任务，写入 `doc/plan.md`

**PM 规划**（需有 PM + Dev Agent）：
1. 委托 `{代号}-pm` 分析需求，**追问边界条件直到用户确认所有细节**
2. 追问完成后，PM 输出 PRD 到 `doc/prd.md`
3. 委托 `{代号}-dev` 根据 PRD 出技术方案，写入 `doc/tech-plan.md`，展示给用户确认
...
**循环结束判定**：
- PASS → 标记 ✅
- 第 3 轮仍 FAIL → 标记 ⚠️（低质量通过），向用户报告

---

## 收尾

全部任务完成后，最终更新 CLAUDE.md，写入统计日志，向用户报告。
